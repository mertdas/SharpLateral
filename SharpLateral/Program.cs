    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            if (command == "redwmi")
            {
                ...
                RedWMI.Redwmi1(args.Skip(1).ToArray()); // Skip the command itself
            }
            if (command == "redexec")
            {
                // Example usage: Latrealmove redexec 192.168.1.2 C:\localfile.exe C:\remotefile.exe MyServiceName
                RedExec.Redexec(args.Skip(1).ToArray()); // Skip the command itself
            }
            if (command == "schedule")
            {
            ...
            else // Use 'else' here to properly handle unrecognized commands
            {
               //boom
            }
        }
